...
fn isTestableZigFile(entry: std.fs.Dir.Walker.Entry) bool {
    if (entry.kind != .file) {
    ...
    if (std.mem.eql(u8, "build.zig", entry.basename)) {
        std.debug.print("FALSE: entry.path: {s}\n", .{entry.path});
