...
pub inline fn parseSingleLineSingleValue(key: []const u8, line: []const u8) ![]const u8 {
...
test "OK" {
    ...
    for (KeyValues) |kv| {
        ...
        while (lines.next()) |line| {
            resolved_value = try parseSingleLineSingleValue(@tagName(kv.key), line);
...
test "empty value" {
    const resolved_value = try parseSingleLineSingleValue(@tagName(Token.FILE), @tagName(Token.FILE));
...
test "fail TokenError.KeyNotFoundError" {
    ...
    try std.testing.expectError(TokenError.KeyNotFound, parseSingleLineSingleValue(@tagName(Token.FILE), line));
}
