...
pub inline fn parseMultiLineMultiValue(allocator: std.mem.Allocator, input: []const u8, pos: *usize, stop: []const u8) ParseError![]const []const u8 {
    ...
    std.debug.print("{s}:\tpos/stop_pos_rel/stop_pos_abs: {d}/{d}/{d}\n", .{ stop, pos.*, stop_pos_rel, stop_pos_abs });
    std.debug.print(".parseMultiLineMultiValue:\n{s}\n", .{input[pos.*..stop_pos_abs]});

    const value_slice = input[pos.*..stop_pos_abs];
...
//     ;
//     var pos: usize = 0;
//     const uloads = try parseMultiLineMultiValue(allocator, lines, &pos, @tagName(Token.MAXTIME));
//     defer allocator.free(uloads);
//
//     const first_number = "6.9981980E+06";
//     const last_number = "-2.5173898E+05";
//
//     try std.testing.expectEqualSlices(u8, first_number, uloads[0]);
//     try std.testing.expectEqualSlices(u8, last_number, uloads[uloads.len - 1]);
// }
